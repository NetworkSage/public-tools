...
class Interflow():
    def __init__(self, flowdata, is_json=False):
        ...
        if is_json:
                ...
                if not outpkts_vals and not total_outpkts_vals:
                    if not totalpkts_vals or totalpkts_vals[0] == 0:  # 0 total packets makes no sense...
                        if not outbytes_vals:  # if we don't have this we can't even take a guesstimate
                            print("Failed to find valid totalpackets value and outbytes value (which we could've used to guess on number of packets)")
                            missing += ["outpkts_delta"]
                        else:
                            outpkts_vals = [math.ceil(outbytes_vals[0]/max_segment_size)]  # roughly guess on number of outgoing packets
                        ...
                        if not inpkts_vals:
                            if not inbytes_vals:
                                ...
                                print("Failed to find valid inpkts value, so using totalpackets - (inbytes/MSS) to guess on outpkts")
                ...
                if not inpkts_vals and not total_inpkts_vals:
                    if not totalpkts_vals or totalpkts_vals[0] == 0:  # 0 total packets makes no sense...
                        if not inbytes_vals:  # if we don't have this we can't even take a guesstimate
                            print("Failed to find valid totalpackets value and inbytes value (which we could've used to guess on number of packets)")
                            missing += ["inpkts_delta"]
                        else:
                            inpkts_vals = [math.ceil(inbytes_vals[0]/max_segment_size)]  # roughly guess on number of incoming packets
                    else:  # we have a totalpackets count that seemingly makes sense, so try to use it
                        if not outpkts_vals:
                            if not outbytes_vals:
                                ...
                                print("Failed to find valid outpkts value, so using totalpackets - (outbytes/MSS) to guess on inpkts")
            ...
            self.secflow_key = None
        """
        if self.secflow_key is not None:
            print(f"Successfully created genericflow from Interflow! Details:\n"
                  f"{self.source_ip}:{self.source_port} <-> {self.dest_ip}:{self.dest_port}:\n"
                  f"srcPackets: {self.source_pkts} dstPackets: {self.dest_pkts}\n"
                  f"srcBytes: {self.source_bytes} dstBytes: {self.dest_bytes}")
        """